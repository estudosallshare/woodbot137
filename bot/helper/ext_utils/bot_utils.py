...
def bt_selection_buttons(id_: str):
    ...
    buttons = ButtonMaker()
    if WEB_PINCODE:
        buttons.buildbutton("✔️Select Files", f"{BASE_URL}/app/files/{id_}")
        buttons.sbutton("✔️Pincode", f"btsel pin {gid} {pincode}")
    else:
        buttons.buildbutton("✔️Select Files", f"{BASE_URL}/app/files/{id_}?pin_code={pincode}")
...
def get_readable_message():
    with download_dict_lock:
        ...
        buttons.sbutton("✔️Statistics", str(THREE))
        buttons.sbutton("✔️Close", "status close")
        sbutton = InlineKeyboardMarkup(buttons.build_menu(3))
        
        if STATUS_LIMIT is not None and tasks > STATUS_LIMIT:
            msg += f"<b>✔️Tasks:</b> {tasks}\n"
            buttons = ButtonMaker()
            if EMOJI_THEME is True:
